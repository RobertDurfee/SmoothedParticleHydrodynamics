...
GLProgram::GLProgram(uint32_t apl, uint32_t apc, Camera* ac): program_light(apl), program_color(apc), camera(ac) {
    enableLighting();
}
...
void GLProgram::updateMaterial(Vector3f diffuseColor, Vector3f ambientColor, Vector3f specularColor, float shininess, float alpha) const {
